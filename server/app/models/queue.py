...
class Queue(Base):
    ...
    queue_messages = relationship("QueueMessage", back_populates="queue", cascade="all, delete-orphan")
    users = relationship("User", secondary="user_queue", back_populates="queues")  
    topic = relationship("Topic", back_populates="queues")  
    routing_keys = relationship("QueueRoutingKey", back_populates="queue", cascade="all, delete-orphan")  